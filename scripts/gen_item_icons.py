...
ITEM_DIR = PROJECT_ROOT / "game" / "data" / "items"
ICON_DIR = PROJECT_ROOT / "game" / "art" / "ui" / "inventory" / "icons"
ICON_DIR.mkdir(parents=True, exist_ok=True)

def make_prompt(name: str, desc: str) -> str:
    return (
        f"Chinese wuxia RPG game item icon. {name}. {desc} "
        f"Dark fantasy wuxia style. The item centered, detailed, game-ready. "
        f"No text, no labels, no UI frames. "
        f"Pure white (#FFFFFF) background. Isolated item."
    )

items = []
for f in sorted(ITEM_DIR.glob("*.tres")):
...
for i, (tres_path, name, desc) in enumerate(items):
    ...
    print(f"[{i+1}/{len(items)}] {item_id}: {name}")
    sys.stdout.flush()
